...
def parse_args():
    ...
    p.add_argument("--temp-workers", type=int, default=1, help="Workers for SAFE -> temp tile stage. For Jupyter-like stability keep 1; script mode can try 2-4.")
    p.add_argument("--daily-workers", type=int, default=2, help="Workers for daily mosaics (threaded)")
    p.add_argument("--weekly-mode", choices=["full","chunked"], default="full", help="Weekly composite mode. full is faster on high-RAM machines; chunked is safer for low memory.")
    p.add_argument("--weekly-chunk-size", type=int, default=2048, help="Chunk size for weekly composite when --weekly-mode chunked")
...
def download_one_product(product_id, product_name, region, acq_date, token, out_dir, overwrite=False):
...
def build_weekly(daily_df, target_regions, weekly_root, date_from, date_to, weekly_mode="full", chunk_size=2048, overwrite=False, compress=False):
    weekly_rows = []
    for region_name in target_regions:
        ...
        if weekly_mode == "full":
            result = create_weekly_composite_from_daily_full_memory(
            ...
            result = create_weekly_composite_from_daily_chunked(
                daily_paths=daily_paths,
                out_path=out_path,
...
def main():
    ...
    if args.skip_query_download:
        ...
        if not args.username or not args.password:
            raise ValueError("CDSE username/password belum diisi. Pakai --username --password atau env vars CDSE_USERNAME/CDSE_PASSWORD")
        token = get_cdse_access_token(args.username, args.password)
        ...
        for idx, row in scenes.iterrows():
            ...
            try:
                ...
                download_results.append({**row.to_dict(), "zip_path": str(out_path), "status": "ok"})
                ok += 1
            except Exception as e:
                print(f"[FAIL] {row['name']} | {e}")
                download_results.append({**row.to_dict(), "zip_path": None, "status": f"fail: {e}"})
                fail += 1

        print(f"\nDOWNLOAD SUMMARY: ok={ok} fail={fail}")
        downloaded_df = pd.DataFrame(download_results)
        save_csv(downloaded_df, logs_root / f"download_results_{args.date_from}_{args.date_to}.csv")
        downloaded_df = downloaded_df[downloaded_df["zip_path"].notna()].reset_index(drop=True)
